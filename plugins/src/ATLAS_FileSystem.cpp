...
const Disk* File::find_local_disk_on(const Host* host)
{
  ...
  for (auto const& disk : host->get_disks()) {
    ...
    mount_point_ = fullpath_.substr(0, fullpath_.find('/', fullpath_.find('/') + 1));
...
void File::close()
{
  std::vector<int>* desc_table = local_disk_->get_host()->extension<FileDescriptorHostExt>()->file_descriptor_table.get();
...
sg_size_t File::write(sg_size_t size, bool write_inside)
{
  ...
  xbt_assert(size < sg_disk_get_size_free(local_disk_), "Disk storage for '%s' exceeded on '%s'", local_disk_->get_cname(), local_disk_->get_host()->get_cname());
  ...
  auto* parse_content = new std::map<std::string, sg_size_t, std::less<>>();
    ...
    while (std::getline(ss, item, ',')) {
        ...
        if (pos != std::string::npos) {
            std::string path = item.substr(0, pos);
            sg_size_t size = std::stoull(item.substr(pos + 1));
            parse_content->insert({path, size});                                                            
        }
    }
